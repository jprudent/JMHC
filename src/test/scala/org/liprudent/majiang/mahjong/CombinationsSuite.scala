...
class CombinationsSuite extends FunSuite {
  ...
  test("Three suited terminal chow") {
    val hule = HuLe(List(Dui(b5)), List(Chow(c1), Chow(c7), Chow(s1), Chow(s7)), ContextualTile(b5, Discarded,
      NotLastTile), PlayerContext(WestWind, EastWind), Nil)
  ...
  test("Three suited terminal invalid, not three family") {
    val hule = HuLe(List(Dui(c5)), List(Chow(c1), Chow(c7), Chow(s1), Chow(s7)), ContextualTile(b5, Discarded,
      NotLastTile), PlayerContext(WestWind, EastWind), Nil)
    val actual = ThreeSuitedTerminalChows.find(hule)
  ...
  test("Three suited terminal invalid, no pair of 5") {
    val hule = HuLe(List(Dui(b4)), List(Chow(c1), Chow(c7), Chow(s1), Chow(s7)), ContextualTile(b5, Discarded,
      NotLastTile), PlayerContext(WestWind, EastWind), Nil)
    val actual = ThreeSuitedTerminalChows.find(hule)
    val expected = EmptyResult
    assert(actual === expected)
  ...
  test("flower tiles") {
    val hule = HuLe(List(Dui(b3)), List(Chow(b1, b2, b3), Chow(b5, b6, b7), Chow(c5, c6, c7), Chow(c7, c8, c9)),
      ContextualTile(b3, Discarded, NotLastTile), PlayerContext(WestWind, EastWind), Nil, bonus = Bonus(List(fb, sa)))
  ...
  test("closed wait") {
    val melded: List[Chow] = List(Chow(b6, b7, b8), Chow(c7, c8, c9))
    val contextualTile: ContextualTile = ContextualTile(s8, Discarded, NotLastTile)
    val context = PlayerContext(WestWind, EastWind)
    val hule = HuLe(List(Pung(s6), Chow(s7, s8, s9), Dui(c6)), melded, contextualTile, context, List(s8))
  ...
  test("all types") {
    ...
    val melded = List(Pung(ww), Chow(s3, s4, s5))
    val lastTile: ContextualTile = ContextualTile(dg, Discarded, NotLastTile)
    val context = PlayerContext(WestWind, EastWind)
    val hule = HuLe(closed, melded, lastTile, context, Nil)

    val actual = AllTypes.find(hule)
    val expected = Result((closed ::: melded).sorted(OrdFigure))
  ...
  test("single wait") {
    ...
    val hule = HuLe(closed, melded, lastTile, context, List(dg))
  ...
  test("Pung of terminal or honor") {
    ...
    val melded: List[Figure] = List(Pung(b9), Pung(dr))
  ...
  test("Mixed Triple Chow not found on PureTripleChow") {
    ...
    val melded: List[Figure] = List(Chow(b2))
    val lastTile: ContextualTile = ContextualTile(dg, Discarded, NotLastTile)
    val context = PlayerContext(WestWind, EastWind)
  ...
  test("Mixed Triple Chow found") {
    ...
    val melded: List[Figure] = List(Chow(s2))
    val lastTile: ContextualTile = ContextualTile(dg, Discarded, NotLastTile)
    val context = PlayerContext(WestWind, EastWind)
